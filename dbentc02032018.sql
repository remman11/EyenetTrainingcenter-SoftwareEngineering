...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Mar 01, 2018 at 07:08 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tblschedlist`  AS  select `c`.`strCName` AS `strCName`,`c`.`strCDetails` AS `strCDetails`,`s`.`strSDesc` AS `strSDesc`,`t`.`strStatusDesc` AS `strStatusDesc`,date_format(`s`.`datStartDate`,'%M %d,%Y') AS `datStartDate`,date_format(`s`.`datEndDate`,'%M %d,%Y') AS `datEndDate`,`s`.`strSRemarks` AS `strSRemarks`,`t`.`strStatusColor` AS `strStatusColor`,`p`.`strProctorName` AS `strProctorName` from (((`tblsched` `s` join `tblstatus` `t` on((`s`.`intSStatusID` = `t`.`intStatusID`))) join `tblcourse` `c` on((`c`.`intCID` = `s`.`intSCourseID`))) join `tblproctor` `p` on((`p`.`intProctorID` = `s`.`intProctorID`))) ;
